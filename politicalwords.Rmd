...
## Préparation des données : UDpipe

Nous allons ensuite utiliser UDpipe pour préparer nos données. Nous allons l'utiliser pour identifier dans nos textes :
...
Il nous reste alors à préparer les donnés pour nos 3 ensembles de textes :

```{r}
udpipedLRM <- udpipe(articlesLRM$articleContent,udpipeModel)
udpipedEELV <- udpipe(articlesEELV$articleContent,udpipeModel)
udpipedRN <- udpipe(articlesRN$articleContent,udpipeModel)
```
...
library(cowplot)
coocurences <- list()
viewCoocurences <- function(udpipeObjects){
    ...
    wordnetwork <- lapply(coocurences, head,30)
...
graphicLexicalNetworks <- cooc %>%  
  ...
  ggtitle("Les réseaux lexicaux à LRM") +
  theme_void()
